...
public class Rmsa {
	...
	public SustrateNetwork realizarRmsa(SustrateNetwork sustrateNetwork, String sourceNode, 
		...
		try{
			...
			if(sustrateNetwork == null || sourceNode == null || targetNode == null
					|| k == null || slotRequerido == null){
				throw new ValidationsExceptions("Argumentos recibidos nulos");
			} else if(sustrateNetwork.getCantidadFS() == 0
					|| slotRequerido == 0 || sustrateNetwork.getEnlacesFisicos() == null
					|| sustrateNetwork.getEnlacesFisicos().size() == 0){
			...
			for (SustrateEdge sustrateEdge : sustrateNetwork.getEnlacesFisicos()) {
				if(sustrateEdge.getNodoUno() == null || sustrateEdge.getNodoDos() == null){
					throw new ValidationsExceptions("SustrateEdge nulo");
				}
